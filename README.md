...
Please follow the [instruction](https://github.com/facebookresearch/fairseq/tree/main/examples/translation#wmt14-english-to-german-convolutional) of Fairseq to prepare the data.
...
fairseq-train ${data_dir}  \
    ...
    [--seg-tokens 32] [--filter-max-length 512:128] [--filter-ratio 8] \
    \
...
* Training machine translation model from scratch on [WMT14 En-De](./examples/DA-Transformer/wmt14_ende.sh).
* Fine-tuning pre-trained DA-Trasnformer on [SQuAD1.1](./examples/DA-Transformer/squad.sh).
...
fairseq-generate ${data_dir} \
    --gen-subset test --user-dir fs_plugins --task translation_dat \
    --remove-bpe --max-tokens 4096 --seed 0 \
    --model-overrides "{\"decode_strategy\":\"greedy\",\"decode_upsample_scale\":8}" \
...
fairseq-generate ${data_dir} \
    --gen-subset test --user-dir fs_plugins --task translation_dat \
    --remove-bpe --max-tokens 4096 --seed 0 \
    --model-overrides "{\"decode_strategy\":\"lookahead\",\"decode_beta\":1,\"decode_upsample_scale\":8}" \
    --path ${average_checkpoint_path}
...
fairseq-generate ${data_dir} \
    --gen-subset test --user-dir fs_plugins --task translation_dat \
    --remove-bpe --max-tokens 4096 --seed 0 \
    --model-overrides "{\"decode_strategy\":\"lookahead\",\"decode_upsample_scale\":8,\"decode_beta\":1,\"decode_no_consecutive_repeated_ngram\":3, \"decode_no_repeated_ngram\":2, \"decode_top_cand_n\":20,\"decode_top_p\":0.9}" \
    --path ${average_checkpoint_path}
```
...
fairseq-generate ${data_dir} \
    ...
    --remove-bpe --max-tokens 4096 --seed 0 \
    --model-overrides "{\"decode_strategy\":\"viterbi\",\"decode_upsample_scale\":8, \"decode_viterbibeta\":1.0}" \
    --path ${average_checkpoint_path}
...
fairseq-generate ${data_dir} \
    --gen-subset test --user-dir fs_plugins --task translation_dat \
    --remove-bpe --max-tokens 4096 --seed 0 \
    --model-overrides "{\"decode_strategy\":\"jointviterbi\",\"decode_upsample_scale\":8,\"decode_viterbibeta\":1.0}" \
    --path ${average_checkpoint_path}
```
...
fairseq-generate ${data_dir} \
    --gen-subset test --user-dir fs_plugins --task translation_dat \
    --remove-bpe --max-tokens 4096 --seed 0 \
    --model-overrides "{\"decode_strategy\":\"sample\",\"decode_upsample_scale\":8,\"decode_top_cand_n\":5,\"decode_top_p\":0.9,\"decode_temperature\":1,\"decode_no_consecutive_repeated_ngram\":3, \"decode_no_repeated_ngram\":2}" \
    --path ${average_checkpoint_path}
```
...
fairseq-generate  ${data_dir} \
    ...
    --model-overrides "{\"decode_strategy\": \"beamsearch\", \
    	\"decode_upsample_scale\":8, \"decode_beta\": 1, \
        \"decode_alpha\": 1.1, \"decode_gamma\": 0, \
        \"decode_lm_path\": None, \
        \"decode_beamsize\": 200, \"decode_top_cand_n\": 5, \"decode_top_p\": 0.9, \
        \"decode_max_beam_per_length\": 10, \"decode_max_batchsize\": 32, \"decode_max_workers\":1, \"decode_threads_per_workers\":6, \"decode_dedup\": True}" \
    --path ${average_checkpoint_path}
...
fairseq-fastgenerate ${data_dir} \
    ...
    --model-overrides "{\"decode_strategy\": \"beamsearch\", \
        ...
        \"decode_lm_path\": \"/path/to/ngram_lm.arpa\", \
        \"decode_beamsize\": 200, \"decode_top_cand_n\": 5, \"decode_top_p\": 0.9, \
        \"decode_max_beam_per_length\": 10, \"decode_max_batchsize\": 32, \"decode_max_workers\":8, \"decode_threads_per_workers\":6, \"decode_dedup\": True}" \
    --path ${average_checkpoint_path}
```
...
* PDAT (uncased, 127M, trained on 16GB Wikipedia + BookCorpus, 500k steps): [Link](https://cloud.tsinghua.edu.cn/f/b048a7eddd204e2098a7/?dl=1)

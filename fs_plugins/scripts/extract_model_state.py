...
logger = logging.getLogger("extract_model_state")

def main():
    parser = argparse.ArgumentParser(
        description="Extract the model state from trained checkpoints",
    )
    # fmt: off
    parser.add_argument('--input', required=True, help='Input checkpoint file path.')
    parser.add_argument('--output', required=True, help='Output model state file path.')
    ...
    # init_pos = ckpt_pos.clone().zero_()
    # init_pos[2:].copy_(ckpt_pos[1:-1])
    # states["decoder.embed_positions.weight"] = init_pos

    with PathManager.open(args.output, "wb") as f:
        torch.save(states, f)
    print("Finished writing converted state to {}".format(args.output))
